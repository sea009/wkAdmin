...
$(function(){
  /**
   *页面删除图片特效 author jiangx
   */
  $(document).on("click", "input.delImg", function(){
    $(this).parents(".imgBox").fadeOut("slow",function(){
      $(this).remove();  
    });
  });
  $(document).on("blur", "input.money", function(){
    var num = $(this).val();num = parseFloat($.trim(num));
    if (isNaN(num)) {num = 0;}$(this).val(num.toFixed(2));
  });
  $(document).on("blur", "input.integer", function(){
    var num = $(this).val();num = parseFloat($.trim(num));
    if (isNaN(num)) {num = 0;}$(this).val(num.toFixed(0));
  });
});
/**
 *公用删除操作
 *@param url 删除操作连接url
 *@param method 默认为get请求，或ajax请求
 */
function delDataCommon(url, method, msg) {
  if (!msg) {msg = '确认删除？';}
    if (confirm(msg)) {
        if (method == 'ajax') {
            $.ajax({
                ...
                success : function(data){
                  if (!data.msg) {
                    ...
                    if (data.status == 0) {
                        Huimodal_alert(data.msg);
                        return;
                    } else {
                        Huimodal_alert(data.msg, 1000, true);
                    }
                }
            });
        } else {
